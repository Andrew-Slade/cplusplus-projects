...
int main() {
	...
	//note for self: built here | status:sucessful
		...
		//first open the file
		ofstream outFile;
		outFile.open("midterm.txt");
		outFile << setw(12) << left << "Name: "
			<< setw(15) << right << usrName << endl
			<< setw(12) << left << "Base: "
			<< setw(15) << right << base << endl
			<< setw(12) << left << "Exponent: "
		...
		//note for self: built here | status: sucessful
		//usrContinue to get whether to repeat
		cout << "Enter y to repeat n to stop: ";
		cin >> usrContinue;

		//right before the while ignore any enters
		cin.ignore();
	} while (usrContinue == 'y');
